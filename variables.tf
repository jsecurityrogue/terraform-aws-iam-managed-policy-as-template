...
variable "approved_policy_version" {
  type        = string
  description = <<-EOT
    Optional. The IAM policy version ID (e.g. "v10") that this override has
    been reviewed and approved against. When provided, Terraform will error
    at plan time if the live managed policy version differs — indicating AWS
    has published an update that requires operator review before re-running.
    ...
    The current version ID is always available in the source_policy_version
    output. After reviewing an AWS policy update and confirming the override
    still meets governance requirements, update this value to the new version
    ID to unblock the plan.
